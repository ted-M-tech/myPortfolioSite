...
| `github.svg` | Simple Icons | 単色（公式も #181717 のほぼ黒） |
| `perplexity.svg` | Simple Icons | 単色（Offblack は公式パレットの基幹3色の1つ） |
| `azure.svg` | Devicon | フルカラー |
| `aws.svg` | Devicon | フルカラー。**ワードマーク版しか無い**（Devicon に素マーク無し） |
...
| **Canva** | Canva 自身の PDF が Apple と同一構造で禁止。"You may not use the Canva Logo or any other Canva-owned graphic symbol, logo, or icon on or in connection with **web sites**... except pursuant to an express written trademark license from Canva."（2026-07-20 に削除） |
| **Vercel** | ガイドライン上の問題ではなく、本人の判断で削除（2026-07-20） |
...
| GitHub | 概ね可 | 「GitHubプロフィールへのリンクとして」等を明示的に許可。改変・再配布は禁止 |
| Perplexity | **確認不能** | perplexity.ai が自動取得を 403 で拒否。禁止文言は見つかっていないが、確認できていない。Offblack は公式パレットの基幹3色の1つなので、色は改変ではない |
| **AWS / Azure** | **VS Code と同じ** | Simple Icons は forbidden brands list に Amazon と Microsoft を並べて掲載 |
